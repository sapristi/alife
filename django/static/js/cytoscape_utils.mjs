...
export const cola_layout_conf = {
  ...
  edgeLength: function (edge) {
    return edge.hasClass("extension") ? 35 : 70;
  },
  infinite: true,
  fit: false,
};
...
export const setup_pnet_cy = function (elements, eventHandler) {
  ...
  const select_edges = (node) => {
    node.incomers("edge").forEach((edge) => edge.select());
    node.outgoers("edge").forEach((edge) => edge.select());
  };

  cy.on("select", ".place,.transition", function (evt) {
    eventHandler(evt.target.data());
    select_edges(evt.target);
  });

  cy.on("unselect", "node", function (evt) {
    eventHandler({ id: null });
  });
  ...
  const update_pnet = function (pnet) {
  ...
  const replace_elements = (newElements) => {
    cy.elements().remove();
    cy.add(newElements);
    const layout = cy.layout(cola_layout_conf);
    return layout;
