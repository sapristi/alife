...
let db_to_string_result res =
  match res with Ok res -> Ok res | Error r -> Error (Caqti_error.show r)
...
let set_sandbox (sandbox : Sandbox.t) (req : Opium.Request.t) =
  req |> Opium.Request.to_json_exn >|= Sandbox.signature_of_yojson
...
module Mol_req = struct
  (** generic result: bact *)
  let get_bact (sandbox : Sandbox.t) = `Json (Bacterie.to_sig_yojson !sandbox)

  let add_mol (sandbox : Sandbox.t) req =
    let mol = Opium.Request.query_exn "mol" req in
    Bacterie.add_molecule mol !sandbox |> Bacterie.execute_actions !sandbox;
  ...
  let pnet_ids_from_mol (sandbox : Sandbox.t) req =
    ...
    let pnet_ids_json = `List (List.map (fun i -> `Int i) pnet_ids) in
  ...
  let execute_pnet_action (sandbox : Sandbox.t) req =
    ...
    ( ( req |> Opium.Request.to_json_exn >|= pnet_action_of_yojson
      >|=? fun pnet_action ->
        let pnet =
          (Reactants_maps.ARMap.find mol pnet_id !sandbox.areactants).pnet
        in
    ...
    >|=? fun (pnet_action, (pnet : Petri_net.t)) ->
      ...
      | Update_token (token_o, place_index) ->
          (match token_o with
          | Some token -> Place.set_token token pnet.places.(place_index)
...
module Env_req = struct
  let get_environment (sandbox : Sandbox.t) req =
    ...
    `Json (Environment.to_yojson !(!sandbox.env)) |> Lwt.return

  let set_environment (sandbox : Sandbox.t) (req : Opium.Request.t) =
    req |> Opium.Request.to_json_exn >|= Environment.of_yojson >|= Result.get_ok
    >|= fun env ->
    !sandbox.env := env;
...
module Reactions_req = struct
  let get_reactions (sandbox : Sandbox.t) req =
    `Json (!sandbox.reac_mgr |> Reac_mgr.to_yojson) |> Lwt.return

  let next_reactions (sandbox : Sandbox.t) req =
    let n = Opium.Request.query_exn "n" req |> int_of_string in
    for i = 0 to n - 1 do
  ...
  let next_reactions_lwt (sandbox : Sandbox.t) req =
    ...
    >|= fun () -> `Json (Bacterie.to_sig_yojson !sandbox)
...
module BactSignatureDB_req = struct
  ...
  let set_from_sig_name (sandbox : Sandbox.t) db_conn req =
    ...
    >|=? fun ({ data; _ } : Yaac_db.BactSig.FullType.t) ->
      sandbox :=
        Bacterie.from_sig data ~env:!(!sandbox.env)
          ~randstate:!(!sandbox.randstate);
      Ok `Empty )
    >|= fun res -> `Res res

  type post_item = { name : string; description : string } [@@deriving yojson]

  let add (sandbox : Sandbox.t) db_conn (req : Opium.Request.t) =
    ( req |> Opium.Request.to_json_exn >|= post_item_of_yojson >|= Result.get_ok
...
module EnvDB_req = struct
  ...
  let load_env (sandbox : Sandbox.t) db_conn req =
    ...
    ( Yaac_db.Environment.find_res (db_conn ()) env_name >|=? fun { data; _ } ->
      !sandbox.env := data;
      Ok `Empty )
    >|= fun res -> `Res res
  ...
  type post_item = {
    name : string;
    description : string;
    data : Bacterie_libs.Environment.t;
  }
  [@@deriving yojson]
