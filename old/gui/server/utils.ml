let add_prefix prefix routes =
  List.map
    (fun (meth, route, callback) -> (meth, prefix ^ route, callback))
    routes

module Resp = struct
  let error_to_response e =
    `String (`Assoc [ ("error", `String e) ] |> Yojson.Safe.to_string)

  let json_to_response j = `String (j |> Yojson.Safe.to_string)
  let default_header = Cohttp.Header.of_list []

  let json_h =
    Cohttp.Header.add default_header "Content-Type" "application/json"
  ...
  let rec handle r =
    ...
    | `Json (j : Yojson.Safe.t) ->
        j |> json_to_response |> Opium.Response.of_json
