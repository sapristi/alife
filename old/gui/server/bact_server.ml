...
let set_log_level cgi : string =
  ...
  match
    cgi#argument_value "level"
    |> Logging_internals.Logging_types.level_of_string
  with
  | Ok level ->
...
let build_all_from_mol (req : Opium.Request.t) =
  match%lwt req |> Opium.Request.to_json_exn >|= Molecule.of_yojson with
  | Ok mol ->
      ...
      let pnet_json =
        ...
        | None -> `Null
      in
      `Json (`Assoc [ ("prot", prot_json); ("pnet", pnet_json) ]) |> Lwt.return
  | Error s -> `Error s |> Lwt.return

let build_all_from_prot (req : Opium.Request.t) =
  let%lwt body = req |> Opium.Request.to_plain_text in

  logger#debug "Received %s" body;
  match body |> Yojson.Safe.from_string |> Proteine.of_yojson with
  | Ok prot ->
      ...
      let pnet_json =
        ...
        | None -> `Null
      in
      `Json (`Assoc [ ("mol", mol_json); ("pnet", pnet_json) ]) |> Lwt.return
  | Error s -> `Error s |> Lwt.return

open Lwt.Infix

type build_req_body = Mol of Molecule.t | Prot of Proteine.t
[@@deriving yojson]
            ...
            [
              `Assoc
                [
                  ...
                  ( "acids",
                    `List
                      (List.map Types.Acid.acid_to_yojson
                         Types.Acid.Examples.nodes) );
                ];
              `Assoc
                [
                  ("type", `String "inputs_arcs");
                  ( "acids",
                    `List
                      (List.map Types.Acid.acid_to_yojson
                         Types.Acid.Examples.input_arcs) );
                ];
              `Assoc
                [
                  ("type", `String "outputs_arcs");
                  ( "acids",
                    `List
                      (List.map Types.Acid.acid_to_yojson
                         Types.Acid.Examples.output_arcs) );
                ];
              `Assoc
                [
                  ...
                  ( "acids",
                    `List
                      (List.map Types.Acid.acid_to_yojson
                         Types.Acid.Examples.extensions) );
                ];
            ] );
      ]
  in
...
let make_routes simulator sandbox db_uri =
  ...
  @ Utils.add_prefix "/api/sandbox"
      (Sandbox_req_handler.make_routes sandbox db_uri)
