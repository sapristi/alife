...
module Molecule =
  struct
    ...
    let of_string s =
      let m_array = Array.init (String.length s)
                  (fun i -> Atome.of_char s.[i]) in
      Array.to_list m_array
...
molecule     *)      
    ...
    let rec extract_message (mol : t) : (string*t) =
      match mol with
      | F::F::F::mol' ->  "",  mol'
      | a::mol' ->
         let (s, mol'') = extract_message mol' in
         (Atome.to_string a)^s,mol'' 
      | [] -> failwith "can't read message in mol"
            
    let rec extract_message_as_mol ( mol : t) : (t*t) =
      match mol with
      | F::F::F::mol' ->  [],  mol'
      | a::mol' ->
         let (msg, mol'') = extract_message_as_mol mol' in
         a::msg,mol'' 
      | [] -> failwith "can't read message in mol"
              
    let string_to_atome_list (s : string) : t =
    ...
    let of_yojson (json : Yojson.Safe.json) : ((t, string) result)  =
      match json with
      | `String mol_str ->
         Ok (string_to_atome_list mol_str)
