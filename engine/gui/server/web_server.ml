...
let add_cors_header =
  let filter : (Opium.Request.t, Opium.Response.t) Rock.Filter.simple =
   fun handler req ->
    let handler' req =
...
let log_in_out =
  let filter : (Opium.Request.t, Opium.Response.t) Rock.Filter.simple =
   fun handler req ->
    ...
    let target = req.target and meth = Opium.Method.to_string req.meth in
    ...
    let handler' req =
      ...
      with _ as e ->
        ...
        logger#error ~tags:[ c ]
          "An error happened while treating the request %s:%s:\n%s\n%s" meth
          target backtrace (Printexc.to_string e);
        `Error
          (Printf.sprintf "An error happened while treating the request at %s"
             target)
...
let index_redirect =
  let filter : (Opium.Request.t, Opium.Response.t) Rock.Filter.simple =
   fun handler req ->
    if List.mem req.target index_resources then (
...
let single_index_redirect =
  let filter : (Opium.Request.t, Opium.Response.t) Rock.Filter.simple =
   fun handler req ->
    ...
    if List.mem req.target pages then (
...
let format_meth meth =
  if meth == Opium.App.get then "GET   "
  else if meth == Opium.App.post then "POST  "
  else if meth == Opium.App.put then "PUT   "
  else if meth == Opium.App.delete then "DELETE"
  else "unknown"

let run port files_prefix routes =
  logger#info "Webserver running at http://localhost:%i" port;
  Opium.App.empty |> Opium.App.port port
  ...
  |> Opium.App.get "/ping" (fun x ->
         Opium.Response.of_plain_text "ok" |> Lwt.return)
  |> List.fold_right
       (fun (meth, route, f) x ->
         logger#info "%s :: %s" (format_meth meth) route;
         (meth route (fun req -> f req >|= Utils.Resp.handle)) x)
       routes
