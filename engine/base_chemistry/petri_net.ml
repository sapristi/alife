...
let make_from_prot uid (prot : Proteine.t) (mol : Molecule.t) : t option =
  ...
  try
    ...
    let places : Place.t array =
      ...
      Array.init (List.length !psigs) (fun index ->
    ...
    let (transitions : Transition.t array) =
      ...
      Array.init (List.length !tsigs) (fun index ->
    ...
    let filter =
      if Config.build_inactive_pnets then fun p_l t_l -> p_l > 0
      else fun p_l t_l -> p_l > 0 && t_l > 0
    in
    if filter (Array.length places) (Array.length transitions) then
      Some
        {
          mol;
          transitions;
          places;
          uid;
          launchables_nb = Q.to_int launchables_nb;
        }
    else None
  with _ as e ->
    (
...
let launch_random_transition randstate (p : t) : Place.transition_effect list =
  let launchables =
    CCArray.filter (fun (t : Transition.t) -> t.launchable) p.transitions
  in
  if Array.length launchables > 0 then
...
let grab (mol : Molecule.t) (pos : int) (pid : int) (pnet : t) : bool =
...
let can_grab (mol : Molecule.t) (pnet : t) : bool =
  Array.fold_left
    (fun res place ->
      ...
      | None -> res
      | Some g -> (
          match Graber.get_match_pos g mol with None -> res | Some _ -> true))
    false pnet.places
...
let can_react (mol1 : Molecule.t) (opnet1 : t option) (mol2 : Molecule.t)
    (opnet2 : t option) =
