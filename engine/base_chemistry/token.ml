...
type option_t = t option [@@deriving show, yojson]

let is_empty ((_, m) : t) = m = ""
let make_empty () : t = (0, "")
let make_at_mol_start (mol : Molecule.t) : t = (0, mol)

let make_at_mol_cut (mol1 : Molecule.t) (mol2 : Molecule.t) : t =
  (String.length mol1, mol1 ^ mol2)

let make (mol : Molecule.t) (pos : int) : t = (pos, mol)
let get_mol ((_, m) : t) : Molecule.t = m

let move_mol_forward ((i, m) : t) : t =
  if i < String.length m then (i + 1, m) else (i, m)

let move_mol_backward ((i, m) : t) : t = if i > 0 then (i - 1, m) else (i, m)

let cut_mol ((i, m) : t) : t * t =
  let m1 = Str.string_before m i and m2 = Str.string_after m i in
  ((i, m1), (0, m2))
...
let get_label ((i, m) : t) =
  if i = String.length m then "" else Char.escaped m.[i]
