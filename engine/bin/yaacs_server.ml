...
type params = {
  ...
  stats : bool; [@default false]  (** Generates running stats *)
  log_level : Logging.level option;
      [@enum
        [
          ("debug", Logging.Debug);
          ("info", Info);
...
let run_yaacs p : unit =
  ...
  if p.log_config = "" then
    Logging.load_global_config_str Config.default_log_config_str
  ...
  let db_conn () = Yaac_db.get_conn db_uri in
  Yaac_db.init db_uri p.data_path
  >>= (fun () ->
        Lwt.join
          [
            Web_server.run p.port p.static_path
              (Bact_server.make_routes (Simulator.make ())
                 (Sandbox.make_empty ()) db_conn)
            >|= ignore;
            Ws_server.run pipe ();
          ])
  |> Lwt_main.run
