...
let empty_sig =
  {
    ...
    randstate =
      { Random_s.seed = 8085733080487790103L; gamma = -7046029254386353131L };
  }
...
let to_yojson s = s |> to_signature |> signature_to_yojson
let of_yojson json = json |> signature_of_yojson |> Result.map of_signature
